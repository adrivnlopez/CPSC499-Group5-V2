...
const createWorkoutPlan = async (req, res) => {
  ...
  try {
    ...
    // Clean and validate exercises, including drop sets
    const formattedExercises = exercises.map((exercise, index) => ({
      exerciseName: exercise.exerciseName,
      sets: exercise.sets,
      reps: exercise.reps,
      weight: exercise.weight,
      dropSets: exercise.dropSets?.map((dropSet, i) => ({
        setNumber: index + 1 + (i + 1), // Increment for drop sets
        reps: dropSet.reps,
  ...
  try {
    ...
    if (exercises && Array.isArray(exercises)) {
      exercises.forEach((newExercise) => {
        ...
        const isDuplicate = plan.exercises.some(
          (existingExercise) =>
            existingExercise.exerciseName === newExercise.exerciseName &&
            existingExercise.sets === newExercise.sets &&
            existingExercise.reps === newExercise.reps &&
            existingExercise.weight === newExercise.weight
        );
    
        if (!isDuplicate) {
          plan.exercises.push(newExercise); // Append unique exercises
        }
      });
    }    
