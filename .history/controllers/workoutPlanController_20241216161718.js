...
const updateWorkoutPlan = async (req, res) => {
  ...
  try {
    ...
    // Process exercises: flatten the regular sets and drop sets
    if (exercises && Array.isArray(exercises)) {
      const formattedExercises = exercises.flatMap((exercise) => {
        ...
        // Add drop sets (ignore `sets` field here)
        if (exercise.dropSets && Array.isArray(exercise.dropSets)) {
          exercise.dropSets.forEach((dropSet, index) => {
            allSets.push({
              exerciseName: `${exercise.exerciseName} (Drop Set)`,
              setNumber: exercise.sets + index + 1, // Continue numbering
              reps: dropSet.reps,
              weight: dropSet.weight,
            });
          });
